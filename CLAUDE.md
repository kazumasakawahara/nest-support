...
3. **Shared Libraries** (`lib/`):
   - `db_operations.py`: Neo4j 接続・クエリ実行・CRUD・仮名化出力
   - `db_new_operations.py`: グラフ構造化登録（embedding 自動付与フック付き）
   - `embedding.py`: Gemini Embedding 2 によるベクトル生成・セマンティック検索・OCR
...
├── lib/                           # 共有Pythonライブラリ
│   ├── db_operations.py           # Neo4j接続・クエリ実行・CRUD・仮名化出力
│   ├── db_new_operations.py       # グラフ構造化登録（embedding自動付与）
