...
5. **Manifesto** (`manifesto/`): 理念・プロトコル・ワークフロー
...
- **Neo4j MCP** (`@alanse/mcp-neo4j-server`): Claude ↔ Neo4j 接続
...
├── field-ui/                      # 現場UI（モバイルファーストPWA）
│   └── server.py                  # FastAPI サーバー（port 8001: /record, /dashboard, /voice）
├── sos/                           # SOS緊急通知サービス
